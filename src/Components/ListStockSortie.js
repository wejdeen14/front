...
function ListStockSortie() {
    ...
    useEffect(() => {
        axios.get("http://localhost:8080/prodSortie")
            ...
            .catch((error) => {
                console.error('Erreur lors de la récupération des bondes de sortie :', error);
            });
    }, []);
            ...
            if (result.isConfirmed) {
                axios.delete(`http://localhost:8080/prodSortie/annulationSortie/${id}`)
                    .then(() => {
                        ...
                        setBondeSorties(bondeSorties.filter(sortie => sortie.id_sortie !== id));
                        Swal.fire({
    ...
    return (
        <div className='bg-white px-4 pt-3 pb-4 rounded-sm border border-gray-200 flex-1 h-screen'>
            ...
            <div className='mt-3'>
                <MDBBtn style={{ color: 'white', background: '#4F86C6', marginBottom: '10px' }} onClick={handleAjoutSortie}>
                    Ajouter Produit
                </MDBBtn>
                <table className='w-full bg-white border border-gray-200 divide-y divide-gray-200 m-0'>
                    <thead className='bg-gray-50'>
                        <tr>
                            ...
                            <th scope="col" className="px-1 py-2 text-left text-xs font-medium text-gray-500 uppercase fw-bold tracking-wider" style={{ width: '15%', fontSize: "15px" }}>Montant </th>
                            ...
                            <tr key={index}>
                                ...
                                <td className="px-1 py-2 whitespace-nowrap" style={{ width: '15%' }}>
                                    {bondeSortie.sortie.map((detail, idx) => (
                                        <div key={idx}>{detail.nomProd}</div>
                                    ))}
                                </td>
                                <td className="px-1 py-2 whitespace-nowrap" style={{ width: '15%' }}>{bondeSortie.montantsortie}</td>
                                ...
                                <td className='flex mt' style={{ width: '10%' }}>
                                    <span style={{ marginRight: '10px' }}>
                                        <Link to={`/ShowSortie/${bondeSortie.id_sortie}`}>
                                    ...
                                    <span style={{ marginRight: '10px' }}>
                                        <BiEdit
                                            ...
                                            onClick={() => handleAnnuler(bondeSortie.id_sortie)}
                                        />
                                    </span>
                                    <span style={{ marginRight: '10px' }}>
                                        <BsXCircle
